...
GROUP BY membercost
ORDER BY membercost < ( monthlymaintenance /5 ) DESC 
...
the member name. */
SELECT DISTINCT CONCAT( firstname,  " ", surname ) AS membername, name
...
SELECT Facilities.name AS facility, CONCAT( Members.firstname,  ' ', Members.surname ) AS membername, 
...
GROUP BY name
HAVING SUM(CASE WHEN memid = 0 THEN guestcost * slots ELSE membercost * 
slots END) < 1000
